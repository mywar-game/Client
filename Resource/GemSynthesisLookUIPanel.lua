...
GemSynthesisLookUIPanel = {
panel = nil,
}

function GemSynthesisLookUIPanel:New(o)
    o = o or {}
    setmetatable(o,self)
    self.__index = self
   return o
...
function GemSynthesisLookUIPanel:Create(para)
    local p_name = "GemSynthesisLookUI"
	...
	local materilsToProduct = DataManager.getSystemGemSynthesisAllOreMaterialByType(GameField.GemSynthesisOre)
	local reqData = {}
	local myMaterial
	local netData = nil
	...
	local allData = DataManager.getSystemGemstoneForgeList()
	for k, v in pairs(allData) do
		local data = DataManager.getSystemGemstone(v.gemstoneId)
		if v.type == GameField.GemSynthesisForge and v.needLevel <= level then			
			table.insert(showData,data)
		end
	end

	--进入锻造内容	
	local function enterForgeUI()
		if nil ~= selectItem.data then
			LayerManager.show("GemSynthesisUIPanel", {data = selectItem.data})
		end
	end
	-------------------------------
	...
	local function showItemForge(item, data)
		...
		if nil == materilsToProduct[data.toolId].toolId then
			return
		end
	...
	local function touchResponse(sprite, data)
		...
		if num < 1 then
			...
			LayerManager.show("InputCountUIPanel", {callBack = doExe, arguments = {item = sprite, data = data, baseNum = baseNum}, upLimit = num, lowLimit = 1}, true)
		end
	end
	
	local function equipTouchCallBack(para)
		if 3 ~= currrentOperate   or  nil == materilsToProduct[para.data.toolId] then
			return
		end
        cacheData = para.data
        ...
        elseif para.type == GameField.Event_Move then--处理拖动
            if not cacheMoveSprite then
                ...
                self.panel.layer:addChild(cacheMoveSprite,0x00ff)
            end
	...
	local packagePanel = PackageUIPanel:New()
	local lineSprite = self.panel:getChildByName("img_line1")
	local package = packagePanel:Create({x=545,y=3,equipTouchCallBack=equipTouchCallBack, toolTouchCallBack=equipTouchCallBack, getPageckeToolList = getPageckeToolList, disEnable = true})
	lineSprite:addChild(package.layer)

	local function OnItemShowCallback(scroll_view,item,data)
		...
		self.panel:setItemLabelText(item, "lab_num", "lv." .. data.level)
        
		local iconSprite = IconUtil.GetIconByIdType(GameField.gemstone,data.gemstoneId)
	...
	local function refreshForgeToolInfo()	
		currrentOperate = paraToolId
		otherBgSprite:setVisible(false)
		self.panel:setNodeVisible("ListView", true)
		self.panel:setBtnEnabled("btn_weapen1", false)
		self.panel:setBtnEnabled("btn_weapen2", true)
		self.panel:setNodeVisible("btn_look", false)
        self.panel:InitListView(showData, OnItemShowCallback, OnItemClickCallback, nil, nil, 2)
	end

	refreshForgeToolInfo()
	
		--熔炼
	local function oreSmelt()
		self.panel:setBtnEnabled("btn_weapen1", true)
		self.panel:setBtnEnabled("btn_weapen2", false)
		self.panel:setNodeVisible("ListView", false)
		self.panel:setNodeVisible("btn_look", true)
		otherBgSprite:setVisible(true)		
	end
	...
	local function reqSmelt(para)
		...
		req:setInt_num(num)
		req:setInt_forgeType(GameField.GemSynthesisOre)
	...
	local function showThinkingSlider(para)
		...
		local labelTTFPercent = cc.LabelTTF:create(LabelChineseStr.GemSynthesisLookUISliderName_1, "Arial",20, {width = 0, height = 0})
		...
		-- 接收网络消息
	function GemSynthesisLookUIPanel_GemstoneAction_gemstoneForge(msgObj)
		...
		elseif 2 == netData.arugments.status then
			...
			Tips(LabelChineseStr.GemSynthesisLookUITip_1)
	...
	self.panel:addNodeTouchEventListener("btn_weapen1",btnCallBack, 2)
	self.panel:addNodeTouchEventListener("btn_weapen2",btnCallBack, 3)

    return panel
end

--退出
function GemSynthesisLookUIPanel:Release()
...
--隐藏
function GemSynthesisLookUIPanel:Hide()
	self.panel:Hide()
end

--显示
function GemSynthesisLookUIPanel:Show()
	self.panel:Show()
